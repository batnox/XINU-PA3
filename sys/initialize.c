...
nulluser()				/* babysit CPU when no one is home */
{
	...
	//kprintf("MAIN pdbr %d",proctab[userpid].pdbr);
	resume(userpid);
	//kprintf("MAIN pdbr %d",proctab[userpid].pdbr);
	while (TRUE)
		/* empty */;
...
*/
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	init_bsm();
	init_frm();
	create_global_pagetables();
	set_evec(14,(unsigned) pfintr);
	create_pagedir(NULLPROC);
	pd_t *nullpd = (pd_t*)(proctab[NULLPROC].pdbr);
	//kprintf("NULLPROF pdbr %d first 4 entries %d %d %d %d", proctab[NULLPROC].pdbr,nullpd[0].pd_base, nullpd[1].pd_base, nullpd[2].pd_base, nullpd[3].pd_base);
	write_cr3(proctab[NULLPROC].pdbr);

	//srpolicy(SC);
	debug_on=0;
	page_replace_policy=SC;
	//kprintf("page policy %d\n", page_replace_policy);
		sc_head = (sc_frames *) getmem(sizeof(sc_frames));
	enable_paging();
